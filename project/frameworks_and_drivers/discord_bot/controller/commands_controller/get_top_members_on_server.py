...
@MW_BOT.bot.command()
async def get_top_members_on_server(ctx: commands.Context, chart: str = "no"):
    ...
    URL: str = os.getenv("BASE_URL") + f"/member/top_members?server_id={server.id}&member_id={author.id}&channel_id={-1}&from_date={from_date}&by=server" #<--- channel_id as -1 because we don't need to inform any channel here
    ...
    #Path to the image
    IMG_PATH: str = MembersGraph.REPO_PATH + f"/best_members_by_server_{server.id}{author.id}.png"
    ...
    emb, file = get_emb_without_author(
        title = "Top 5 most active members in the server",
        desc = f"",
